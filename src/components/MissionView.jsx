...
export default function MissionView({mission,gps,gpsError,onBack}){
  ...
  useEffect(()=>{
    if(!analysisLocation||!journeyDestination){setRouteData(null);return;}
  ...
  const routeSynthesis=privateReading?.routeSynthesis||null;
  const moonForecast=privateReading?.moonForecast||null;
    ...
    {view==='map'&&<div className={`${mapFullscreen?'fullscreen-map-wrap':'mission-map-wrap'} ${fullscreenSettling?'map-transitioning':''}`}>
      ...
      {mapFullscreen&&<aside className={`fullscreen-prediction-box ${fullscreenPredictionOpen?'open':'collapsed'}`} aria-label="Fullscreen predictions">
        ...
        {fullscreenPredictionOpen&&<div className="fullscreen-prediction-content">
          ...
          {routeSynthesis&&<div className="journey-synthesis-grid"><div className="journey-synthesis-card"><small>Self / mindset</small><p>{routeSynthesis.self||'—'}</p></div><div className="journey-synthesis-card"><small>Journey / movement</small><p>{routeSynthesis.journey||'—'}</p></div><div className="journey-synthesis-card"><small>Encounters / others</small><p>{routeSynthesis.encounters||'—'}</p></div></div>}
          {moonForecast&&<div className="fullscreen-journey-summary"><small>Moon state</small><p>{moonForecast.mindset} {moonForecast.routeText}</p></div>}
