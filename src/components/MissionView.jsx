  ...
  useEffect(()=>{
    ...
    loadGoogleMaps().then(maps=>{
      ...
      service.route({origin:analysisLocation,destination:journeyDestination,travelMode:mode,provideRouteAlternatives:false},(result,status)=>{
        if(cancelled)return;
        if(status!==maps.DirectionsStatus.OK||!result?.routes?.length){setRouteData({status:'FALLBACK',error:`Road directions unavailable (${status||'unknown'}).`});return;}
